...
Backbone.Marionette.CollectionView.prototype.attachBuffer =  function(collectionView, buffer) {
	...
	collectionView.setElement(parseEl($(collectionView.childView.prototype.template)), true);
...
Backbone.Model.prototype.saveChanges = function (success, error) {
...
Backbone.Select2Collection = Backbone.Collection.extend({
	dataForSelect2: function () {
		return this.map(function (t) {
			return t.toSel2Member();
		});
	},
});
...
Backbone.ExtModel = Backbone.Model.extend({
	...
	bindCollection: function(name, collection, matches) {
		...
		if (_.isString(matches)) {
			func = function(model) {
				return model.get(matches) === that.id;
			};
		} else {
			func = matches;
		}
		...
		this.on("change:" + name, function() {
		    that.get(name).reset(collection.filter(func));
		});
	},
	bindManyToMany: function (name, collection, keyName, pairToKeyName) {
		//					Groups, groups   ,groupIds,		tagIds
		var that = this;
		//синхронизация поля с коллекцией
		var matches = function (model) {
			return _.contains(that.get(keyName), model.id);
		};

		var res = new Backbone.FilteredCollection(null, {
			source: collection,
			matches: matches
		});
		...
		this.on("change:" + keyName, function (model, value, options) {

			if (!_.isUndefined(pairToKeyName) && flag) {
				flag = false;
				var prev = model.previous(keyName);
				var current = value;

				var removed = _.difference(prev, current);
				var added = _.difference(current, prev);


				collection.withIds(removed).forEach(function (pair) {
					var keys = pair.get(pairToKeyName);

					var obj = {};
					obj[pairToKeyName] = _.without(keys, model.id);
				...
				collection.withIds(added).forEach(function (pair) {
					var keys = pair.get(pairToKeyName).slice(0);

					keys.push(model.id);

					var obj = {};
					obj[pairToKeyName] = keys;
			...
			//todo: сделать, чтобы не резет, а добавление и удаление, чтобы нге перерендивало всю коллекцию, а делало аппендхтмл
			this.get(name).reset(collection.filter(matches));
			flag = true;
		});
	},
